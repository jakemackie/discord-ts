...
function isDiscordAPIError(error: unknown): error is DiscordAPIError {
	return error !== null && typeof error === 'object' && 'code' in error && 'status' in error && 'message' in error;
}
...
export default {
	...
	async execute(interaction: Interaction) {
		...
		try {
			...
			logger.info(`${interaction.user.username} (/) ${interaction.commandName}`);
			...
			if (isDiscordAPIError(error)) {
				...
				switch (error.code) {
					...
					case 50001:
						...
						break;
					case 50013:
						errorMessage = 'Bot lacks permissions to perform this action in this channel.';
						break;
					case 50007:
