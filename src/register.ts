...
import { REST, Routes, type ApplicationCommandData, type ApplicationCommandDataResolvable } from 'discord.js';
...
async function deployCommands(isGlobal: boolean) {
	try {
		...
		console.log(`Successfully reloaded ${data.length} ${isGlobal ? 'global' : 'guild'} application (/) commands.`);
	} catch (error) {
		console.error(error);
	}
}
